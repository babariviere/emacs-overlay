...
let
  mkExDrv = emacsPackages: name: args:
    let repoMeta = super.lib.importJSON (./repos/exwm/. + "/${name}.json");
  ...
  mkGitEmacs = namePrefix: jsonFile:
    ...
    in builtins.foldl' (drv: fn: fn drv) self.emacs [
      ...
      (drv:
        if drv.passthru.nativeComp
        && self.lib.elem "--with-nativecomp" drv.configureFlags then
          drv.overrideAttrs (old: {
            configureFlags = builtins.map (flag:
              if flag == "--with-nativecomp" then
                "--with-native-compilation"
              else
                flag) old.configureFlags;
          })
        else
          drv)
      ...
      (drv:
        let
          result = drv.overrideAttrs (old: {
            passthru = old.passthru // { pkgs = self.emacsPackagesFor result; };
          });
        in result)
    ];

  mkPgtkEmacs = namePrefix: jsonFile:
    { ... }@args:
    (mkGitEmacs namePrefix jsonFile args).overrideAttrs (old: {
  ...
  emacsGcc = mkGitEmacs "emacs-gcc" ./repos/emacs/emacs-master.json {
    nativeComp = true;
  };

  emacsPgtk =
    mkPgtkEmacs "emacs-pgtk" ./repos/emacs/emacs-feature_pgtk.json { };

  emacsPgtkGcc =
    mkPgtkEmacs "emacs-pgtkgcc" ./repos/emacs/emacs-feature_pgtk.json {
      nativeComp = true;
    };

  emacsUnstable = (mkGitEmacs "emacs-unstable" ./repos/emacs/emacs-unstable.json
    { }).overrideAttrs
    (old: { patches = [ ./patches/tramp-detect-wrapped-gvfsd-27.patch ]; });
  ...
  emacsPackagesFor = emacs:
    ((super.emacsPackagesFor emacs).overrideScope' (eself: esuper:
      ...
      // super.lib.optionalAttrs (super.lib.hasAttr "nongnuPackages" esuper) {
        ...
        exwm = mkExDrv eself "exwm" { packageRequires = [ eself.xelb ]; };
      }));
